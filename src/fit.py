...
def fit_sum_multi_sine(t, y, yerr, max_n_fit, weights, Omega_0, gamma, rsb=True, gamma_fixed=False,
    ...
    if gamma_fixed == False:
        ...
        p0 = np.concatenate((weights, Omega_0, gamma), axis=None)
        res = fit_leastsq(p0, t, y, yerr, sum_multi_sine, rsb, bounds=bounds)
        gamma_fit = res[0][-1]
        weight_fit = res[0][:max_n_fit]
        weight_fit[-1] = 1 - np.sum(weight_fit[:-1])
        Omega_fit = res[0][-2]
        ...
        p0 = np.concatenate((weights, Omega_0), axis=None)
        res = fit_leastsq(
            p0, t, y, None, sum_multi_sine_fixed_gamma, (rsb, gamma), bounds=bounds)
        gamma_fit = gamma
        weight_fit = res[0][:max_n_fit]
        weight_fit[-1] = 1 - np.sum(weight_fit[:-1])
        Omega_fit = res[0][-1]
